...
pub const CHUNK_BLOCK_WIDTH: usize = 64;
...
impl Chunk {
    pub async fn generate(chunk_i: i64, chunk_j: i64, chunk_k: i64) -> Self {
        let mut c = Self {
    ...
    /// Renders the Chunk.
    pub fn render<'a>(&'a self, render_pass: &mut wgpu::RenderPass<'a>, block_texture_bind_group: &'a wgpu::BindGroup) {
        if let Some(vertex_buffer) = &self.block_mesh_buffer {
            render_pass.set_bind_group(0, block_texture_bind_group, &[]);
            render_pass.set_vertex_buffer(0, vertex_buffer, 0, self.vertex_count as u64);
        }
    }
    ...
    fn make_greedy_mesh(&mut self, _device: &wgpu::Device) {
        ...
        // TODO: copy vertices to the vertex buffer
        // if let Some(vertex_buffer) = &self.block_mesh_buffer {

        // } else {
        //     self.block_mesh_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        //         label: (),
        //         size: (),
        //         usage: (),
        //     });
        ...
        for layer in 0..CHUNK_BLOCK_WIDTH {
            ...
            for i in 0..CHUNK_BLOCK_WIDTH {
                for j in 0..CHUNK_BLOCK_WIDTH {
                    ...
                    // traverse along the "i" axis first, getting the ending_point's `i` (start at
                    // i + 1 because the block at i is the reference_block, which doesn't need to
                    // be matched to itself, silly :))
                    for (k, flag_row) in visited_mask.iter_mut().enumerate().take(CHUNK_BLOCK_WIDTH).skip(i + 1) {
                            ...
                            == reference_block.block_type
                        {
                            ...
                            // this position as visited, just break
                            break;
                        }
                    }
                    ...
                    for l in (j + 1)..CHUNK_BLOCK_WIDTH {
                        ...
                        for (m, flag_row) in visited_mask.iter_mut().enumerate().take(ending_point.0 + 1).skip(i) {
                            if self
                        ...
                        // then we'll move onto the next row
                            ...
                            for row_flags in visited_mask.iter_mut().take(ending_point.0 + 1).skip(i) {
                                row_flags[l] = false;
                            }
