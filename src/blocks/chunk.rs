...
impl Chunk {
    pub async fn generate(chunk_i: i64, chunk_j: i64, chunk_k: i64, device: &wgpu::Device) -> Self {
        let block_mesh_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            // blocks * faces_per_block * vertices_per_face * bytes_per_vertex
            // size is the maximum number of vertices that could be generated by the greedy mesh.
            size: (CHUNK_BLOCK_WIDTH.pow(3) * 6 * 6 * std::mem::size_of::<ChunkMeshVertex>())
                as wgpu::BufferAddress,
            usage: wgpu::BufferUsage::VERTEX,
            label: Some("chunk mesh buffer"),
        });
        let mut c = Self {
    ...
    pub fn logic(&mut self, device: &wgpu::Device, queue: &mut wgpu::Queue) {
        ...
        if self.needs_mesh_update {
            self.make_greedy_mesh(device, queue);
    ...
    pub fn render<'a>(&'a self, render_pass: &mut wgpu::RenderPass<'a>) {
        render_pass.set_vertex_buffer(0, &self.block_mesh_buffer, 0, self.vertex_count as u64);
    }
    ...
    fn make_greedy_mesh(&mut self, device: &wgpu::Device, queue: &wgpu::Queue) {
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("chunk vertex buffer copy encoder"),
        });
        ...
        encoder.copy_buffer_to_buffer(
            ...
            0,
            (self.vertex_count * std::mem::size_of::<ChunkMeshVertex>()) as wgpu::BufferAddress,
        );
